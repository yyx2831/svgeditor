...
<template>
  <button class="back-button" @click="exitPreview" @mousedown.stop>返回</button>
  <div
    class="canvas"
    :style="canvasStyle"
...
<script setup lang="ts">
  import { getCurrentInstance, PropType, reactive, watch, computed } from 'vue';
  ...
  watch(
    () => props.dataModel,
    (newVal) => {
      Object.assign(preview_data, newVal);
    }
  );
  ...
  const canvasStyle = computed(() => {
    return {
      backgroundColor: preview_data.config.background_color,
      backgroundImage: preview_data.config.grid
  ...
  // }
  const exitPreview = () => {
    localStorage.setItem('preview_data', JSON.stringify(preview_data));
    router.go(-1);
  };
