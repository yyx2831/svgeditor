...
<script setup lang="ts">
  ...
  import TopPanel from './components/top-panel/index.vue'; // 引入头部组件
  import LeftPanel from './components/left-panel/index.vue'; // 引入左侧面板组件
  import CenterPanel from './components/center-panel/index.vue'; // 引入中间面板组件
  import RightPanel from './components/right-panel/index.vue'; // 引入右侧面板组件
  import BottomPanel from './components/bottom-panel/index.vue'; // 引入底部面板组件
  import { useSvgEditLayoutStore } from '../../store/svgedit-layout'; // 引入store
  import { onMounted, PropType, reactive, ref } from 'vue';
  import ExportJson from '@/components/webtopo-svg-edit/components/export-json/index.vue'; // 引入导出组件
  import ImportJson from '@/components/webtopo-svg-edit/components/import-json/index.vue'; // 引入导入组件
  import ComponentTree from '@/components/webtopo-svg-edit/components/component-tree/index.vue'; // 引入组件树组件
  import { IVisibleConf, EVisibleConfKey } from './types'; // 引入类型
  import { IConfigCenter } from '@/config-center/types'; // 引入类型
  import { useImportDataModel } from '@/hooks'; // 引入hooks
  import { useGlobalStore } from '@/store/global'; // 引入全局store
  import { IDoneJson } from '@/store/global/types'; // 引入全局store类型
  import 'animate.css'; // 引入动画库
  import { useConfigStore } from '@/store/config'; // 引入配置store
  import vue3RulerTool from '@/components/vue3-ruler-tool/index.vue'; // 引入标尺组件
