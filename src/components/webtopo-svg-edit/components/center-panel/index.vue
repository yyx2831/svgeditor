...
<template>
  ...
  >
    ...
    >
      ...
      <defs>
        <pattern id="pattern_grid" patternUnits="userSpaceOnUse" x="0" y="0" width="10" height="10">
          <rect width="1" height="1" rx="1" ry="1" fill="#ffffff" />
        </pattern>
      </defs>
      <rect v-if="configStore.svg.grid" width="100%" height="100%" fill="url(#pattern_grid)" />
        ...
        >
          <g :class="`${getCommonClass(item)}`">
            ...
            >
              ...
              ></use>
              <template v-else-if="item.type === EDoneJsonType.CustomSvg">
                <span>{{ item.props?.uniqueKey?.val?.length || 0 }}</span>
                <span>
                  {{ item.props }}
                </span>
                999999988888
                <!-- <component
                  v-if="item.keyObj && item.keyObj.tag"
                  :is="item.keyObj.tag"
                  v-bind="prosToVBind(item.keyObj)"
                  width="100"
                  height="100"
                  :id="item.keyObj.id"
                  :transform="`translate(${
                    item.actual_bound.x - 40 + item.actual_bound.width / 2
                  },${item.actual_bound.y + item.actual_bound.height / 2}) scale(${item.scale_x},${
                    item.scale_y
                ...
                <!--<component
                  :is="item.tag"
                  v-bind="prosToVBind(item)"
                  :width="item.props.uniqueKey ? 140 : 100"
                  height="100"
                  :id="item.id"
                  :transform="`translate(${item.actual_bound.x + item.actual_bound.width / 2},${
                    item.actual_bound.y + item.actual_bound.height / 2
                ...
                <component
                  ...
                  v-bind="prosToVBind(item)"
                  :width="
                    item.props?.uniqueKey?.val?.length > 0
                      ? `${item.props.uniqueKey.val.length * 250}`
                      : '100'
                  "
                  height="100"
                  :id="item.id"
...
<script setup lang="ts">
  ...
  onMounted(() => {
    canvasRef.value?.focus();
    setTimeout(() => {
      // 从 localStorage 获取 previewData
      const previewData = JSON.parse(localStorage.getItem('preview_data') || '{}');

      // 覆盖 useSvgEditLayoutStore 的值
      if (previewData.layout_center) {
        console.log('previ2ewData', previewData.layout_center);
        svgEditLayoutStore.center_offset = previewData.layout_center;
      }

      // 覆盖 configStore 的值
      if (previewData.config) {
        console.log('previ2ewData', previewData.config);
        configStore.svg = previewData.config;
      }

      // 覆盖 globalStore 的值
      if (previewData.done_json) {
        console.log('previ2ewData', previewData.done_json);
        globalStore.done_json = previewData.done_json;
      }
    }, 1);
  });
</script>
